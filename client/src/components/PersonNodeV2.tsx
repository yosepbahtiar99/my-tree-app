...
export default function PersonNodeV2({ data }: any) {
  ...
  return (
    <div 
      className={`relative flex flex-row items-center w-[240px] rounded-full p-2 border shadow-sm cursor-pointer ${bgColor} ${showMenu ? 'menu-open !z-[9999]' : ''} ${highlightClass}`}
      onClick={(e) => handleAction(e, 'VIEW_PERSON')}
    >
      ...
      <div className="shrink-0 mr-1" ref={menuRef}>
        <button 
          ...
          className="w-11 h-11 rounded-full flex items-center justify-center bg-black/20 hover:bg-black/40 text-white transition-colors shadow-sm border border-transparent cursor-pointer"
        >
        ...
        {showMenu && (
          <div className="absolute top-12 right-0 bg-white rounded-lg shadow-2xl border border-slate-100 py-1 w-52 z-[99999] overflow-hidden">
            {data.user && (
              <>
                <button 
            ...
            <button 
              ...
              className="w-full text-left px-4 py-3 min-h-[44px] text-sm font-medium text-amber-600 hover:bg-amber-50 transition-colors flex items-center gap-2"
            >
