...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(res.data.message || 'Berhasil mendaftar. Silakan tunggu persetujuan Admin.');
      setEmail('');
      setPassword('');
    } catch (err: any) {
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-sm border border-border/50">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-muted-foreground">
            Daftar untuk menjadi editor. Akses akan dibuka setelah disetujui Super Admin.
          </p>
        </div>
        {error && (
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            >
              {loading ? 'Memproses...' : 'Daftar Jadi Editor'}
            </button>
          </div>
          <div className="text-center text-sm">
            <span className="text-muted-foreground">Sudah disetujui? </span>
